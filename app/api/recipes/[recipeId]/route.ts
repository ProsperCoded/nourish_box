  ...
  try {
    ...
    if (!userId) {
      return NextResponse.json(
        { success: false, message: "User ID is required for authorization (provide as x-user-id header, userId query param, or in body)." },
        { status: 401 }
      );
    }
    ...
    const mediaToDelete: { publicId: string; type: "image" | "video" | "raw"}[] = [];
    ...
    for (const media of mediaToDelete) {
        try {
            ...
            console.log(`Successfully deleted media ${media.publicId} from Cloudinary.`);
        } catch (cloudinaryError) {
            console.error(`Failed to delete media ${media.publicId} from Cloudinary:`, cloudinaryError);
    ...
    return NextResponse.json(
      { success: true, message: "Recipe and associated media deleted successfully." },
      { status: 200 }
    );
    ...
    return NextResponse.json(
      { success: false, message: "Error deleting recipe.", error: errorMessage },
      { status: 500 }
    );
  }
} 