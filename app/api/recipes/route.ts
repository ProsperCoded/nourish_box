...
export async function POST(request: NextRequest) {
  try {
    ...
    const recipesCollection = adminDb.collection(COLLECTION.recipes);
    ...
    return NextResponse.json(
      {
        ...
        }, // Approximate timestamps for immediate client use
      },
      { status: 201 }
    );
  } catch (error) {
  ...
  try {
    ...
    const recipes = recipesSnapshot.docs.map((doc) => {
      ...
      const recipe: Recipe = {
        ...
        createdAt:
          data.createdAt?.toDate().toISOString() || new Date(0).toISOString(), // Default to epoch if undefined
        updatedAt:
          data.updatedAt?.toDate().toISOString() || new Date(0).toISOString(), // Default to epoch if undefined
      };
      return recipe;
    });
