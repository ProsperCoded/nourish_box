...
const footer = () => {
  // Newsletter subscription UI state
  const [email, setEmail] = useState('');
  const [error, setError] = useState('');
  const [message, setMessage] = useState('');
  const [loading, setLoading] = useState(false);

  // Handle input change for newsletter
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setEmail(e.target.value);
  };
  // Subscribe email to mailing list via API
    ...
    <div className='flex flex-col relative overflow-y-scroll lg:flex-row items-center justify-between  lg:px-16 lg:pt-16 lg:pb-32 px-3  text-white bg-[#004c30] '>
      ...
      <div className='flex flex-col justify-between items-center lg:flex-row lg:justify-between text-center lg:text-start w-10/12 '>
        ...
        <div className='flex flex-col justify-end font-inter'>
          ...
          <div className="flex my-2 mb-2 flex-row items-center">
            <input type="email" value={email} placeholder="Enter your email" className=' text-black  mr-2 p-2 rounded-lg ' onChange={handleEmailChange} />
            <button disabled={loading} className={`bg-brand-btn_orange rounded-lg  px-4  text-white py-2 ${loading ? 'opacity-60 cursor-not-allowed' : ''}`} onClick={handleSubscribe}>{loading ? 'Subscribing...' : 'Subscribe'}</button></div>
          {message && <p className='text-green-200 text-sm mb-6'>{message}</p>}
