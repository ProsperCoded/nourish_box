...
const RecipeDetailModal: React.FC<RecipeDetailModalProps> = ({
  ...
  const handleChange = (event: SelectChangeEvent) => setOption(event.target.value);
  ...
  const ingredientsArr = useMemo<string[]>(() => {
    ...
    if (Array.isArray(src)) return src.filter(Boolean).map((s) => String(s).trim());
    ...
    >
      <Box sx={modalStyle}>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200">
          <h2 id="recipe-title" className="text-xl md:text-2xl font-semibold text-gray-900">
            {recipe.name}
          </h2>
          <button
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                    {allMediaItems.map((_, i) => (
                      <button
                        ...
                        className={`h-2.5 w-2.5 rounded-full transition ${i === activeIndex ? "bg-gray-900" : "bg-gray-300 hover:bg-gray-400"
                          }`}
                      />
                    ))}
                  </div>
          ...
          <div className="px-5 py-5 lg:py-6 lg:pr-6 border-t lg:border-t-0 lg:border-l border-gray-200">
            ...
            {recipe.description && (
              <div className="max-w-[56ch]">
                <p id="recipe-description" className="text-[15px] leading-7 text-gray-700">
                  {descExpanded ? recipe.description : truncate(recipe.description, MAX_DESC)}
                </p>
            ...
            {ingredientsArr.length > 0 && (
              <div className="mt-6">
                <h4 className="text-base md:text-lg font-semibold text-gray-800 mb-2">Ingredients</h4>

                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
                  {(ingExpanded ? ingredientsArr : ingredientsArr.slice(0, ING_VISIBLE)).map((ing, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-[15px] leading-6 text-gray-800">
                ...
                {ingredientsArr.length > ING_VISIBLE && (
                  ...
                  >
                    {ingExpanded ? "Show fewer" : `Show ${ingredientsArr.length - ING_VISIBLE} more`}
                  </button>
                )}
              </div>
            )}
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                <button className="px-3 py-2 hover:bg-gray-100" onClick={() => setCount((c) => Math.max(1, c - 1))}>
                  -
                </button>
                <span aria-live="polite" className="px-4 py-2 border-x border-gray-300">
                  {count}
                </span>
                <button className="px-3 py-2 hover:bg-gray-100" onClick={() => setCount((c) => c + 1)}>
                  +
                </button>
              </div>
            </div>
            ...
            <div className="mt-7 pt-5 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <h4 className="text-xs text-gray-500 uppercase tracking-wider">Total Price</h4>
                  <p className="text-2xl font-extrabold text-gray-900">{formatNGN((recipe.price || 0) * count)}</p>
                </div>
                <button
                ...
                >
                  ...
                  {cartLoading ? "Adding..." : (showPackaging && !option) ? "Choose packaging" : "Add to Cart"}
                </button>
              </div>
            </div>
          </div>
