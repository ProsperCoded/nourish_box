...
import { Clock, DollarSign, Star, Edit, Trash2, Eye, Film, ImageIcon } from "lucide-react";
    ...
    >
        ...
        {recipe.displayMedia ? (
          recipe.displayMedia.type === 'video' ? (
            <video
              ...
              muted
              loop
              preload="metadata" // Helps with getting video dimensions quickly
            />
          ) : (
            <Image
              src={recipe.displayMedia.url}
      ...
      <div className="p-4 flex-grow flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-lg font-semibold text-gray-800 line-clamp-1 mr-2 flex-grow" title={recipe.name}>{recipe.name}</h3>
          <div className="bg-gray-100 px-2 py-1 rounded text-xs text-gray-600 whitespace-nowrap">Order: {recipe.order}</div>
        </div>
        
        <p className="text-gray-600 text-sm line-clamp-2 mb-3 flex-grow" title={recipe.description}>{recipe.description}</p>
        ...
        {recipe.samples && recipe.samples.length > 0 && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-2">
              {recipe.samples.slice(0, 4).map((sample, index) => ( // Show up to 4 samples
                <div key={index} className="relative group w-16 h-16 rounded overflow-hidden border border-gray-200">
