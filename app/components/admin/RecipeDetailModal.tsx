...
import { X, Clock, DollarSign, Star, ShoppingBag, ChevronLeft, ChevronRight, PlayCircle, Image as ImageIcon } from "lucide-react";
...
export function RecipeDetailModal({ recipe, onClose }: RecipeDetailModalProps) {
  ...
  const allMediaItems: MediaItem[] = [
    { id: "display", url: recipe.displayMedia.url, type: recipe.displayMedia.type, variant: "Display" },
            ...
            <div className="flex flex-col space-y-3">
              ...
              {allMediaItems.length > 1 && (
                <div className="relative flex items-center">
                  ...
                  <div ref={thumbnailsContainerRef} className="flex gap-2 overflow-x-auto scroll-smooth scrollbar-hide py-1 px-1 flex-grow">
                    {allMediaItems.map((media) => (
                      <div
                        ...
                        title={media.variant || (media.type === 'video' ? 'Video' : 'Image')}
                      >
                        {media.type === "video" ? (
                          <>
                            <video src={media.url} className="w-full h-full object-cover bg-black" preload="metadata" />
            ...
            <div className="space-y-5 lg:max-h-[calc(95vh-100px)] lg:overflow-y-auto lg:pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
              <div className="pb-4 border-b border-gray-200">
                <div className="flex justify-between items-start mb-1">
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 leading-tight">{recipe.name}</h2>
                  {recipe.featured && (
                ...
                <p className="text-gray-600 text-sm sm:text-base leading-relaxed">{recipe.description}</p>
              </div>

              <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm sm:text-base">
                <div className="flex items-center col-span-2 sm:col-span-1">
                  <Clock size={20} className="mr-2.5 text-gray-500 flex-shrink-0" />
                  <span className="text-gray-600">Prep Time:</span>
                  <span className="font-medium text-gray-800 ml-auto pl-2">{formattedDuration}</span>
                </div>

                <div className="flex items-center col-span-2 sm:col-span-1">
                  <DollarSign size={20} className="mr-2.5 text-brand-btn_orange flex-shrink-0" />
                  <span className="text-gray-600">Price:</span>
                  <span className="font-bold text-brand-btn_orange ml-auto pl-2">{formattedPrice}</span>
                </div>
                
                <div className="flex items-center col-span-2 sm:col-span-1">
                  <ShoppingBag size={20} className="mr-2.5 text-gray-500 flex-shrink-0" />
                  <span className="text-gray-600">Display Order:</span>
                  <span className="font-medium text-gray-800 ml-auto pl-2">#{recipe.order}</span>
                </div>
              </div>

              <div>
                <h3 className="text-lg font-semibold text-gray-700 mb-2.5">Ingredients</h3>
                {recipe.ingredients && recipe.ingredients.length > 0 ? (
                  ...
                  <p className="text-gray-500 italic text-sm">No ingredients listed.</p>
                )}
              </div>
            </div>
          </div>
